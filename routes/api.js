...
mongoose.connect('mongodb://a:momo.2005@ds023325.mlab.com:23325/games');
const app = express()
...
router.post('/login', (req, res) => {
  let userDate = req.body;
  User.findOne({ phone: userDate.phone }, (err, User) => {
    if (err) {
      console.log(err);
    } else {
      if (!User) {
...
router.post('/addgames', function (req, res) {
  ...
  router.get('/getgames', function(req, res) {
    Games.find(function(err, review) {
        if (err)
            res.send(err)
        res.json(review);
    });
...
router.get('/newgames',function(req, res) {
        Games.find({ neworused: 0},function(err, review) {
    if (err)
        res.send(err)
    res.json(review);
    });
...
router.get('/usedgames',function(req, res) {
      Games.find({ neworused: 1},function(err, review) {
  if (err)
      res.send(err)
  res.json(review);
  });
  ...
  router.post('/getmyorders',function(req, res) {
      Orders.find({phone:req.body.phone}
      ,function(err, review) {
    if (err)
        res.send(err)
    res.json(review);
...
// //الحصول على جميع اليوزرية
// router.get('/getUsersaa11', function(req, res) {

//     User.find({},function(err, addhavecar1) {
//            if (err)
//                res.send(err)
//            res.json(addhavecar1);
//        });
...
// //حذف المدينة
// router.post('/dellmasterzone', function(req, res) {
//     addplayersids.findOneAndRemove({masterzone:req.body.masterzone},function(err, review) {
//            if (err)
//                res.send(err)
//            res.json(review);
//        });
...
//  //حذف الاقظية 
// router.post('/dellFMZ', function(req, res) {
//     addplayersids.findOneAndRemove({FMZ:req.body.FMZ},function(err, review) {
//            if (err)
//                res.send(err)
//            res.json(review);
//        });
...
//  //حذف المنطقة
// router.post('/dellsubzone', function(req, res) {
//     addplayersids.findOneAndRemove({subzone:req.body.subzone},function(err, review) {
//            if (err)
//                res.send(err)
//            res.json(review);
//        });
